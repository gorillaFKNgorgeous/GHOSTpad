...
def main():
    ...
    with Codex() as codex:
        ...
        if account is not None and getattr(account, "account", None) is not None:
            print("Codex account session is already available.")
            return 0

        login = codex.login_chatgpt_device_code()
        print("Open this URL in a browser:", login.verification_url)
        print("Enter this one-time code:", login.user_code)
        completed = login.wait()
        if not getattr(completed, "success", False):
            print("Codex sign-in did not complete successfully.", file=sys.stderr)
            return 1
        print("Codex account sign-in complete.")
        return 0
